<?php
    ...
    if(isset($_GET['movie']) && strlen($_GET['movie']) > 0)
    {
        $movieCode = $_GET['movie'];
    }

    $movieDB = \MovieDatabaseFactory::create();
    $activeMovie = $movieDB->getMovie($movieCode);

    $errors = [];


    $title          = $activeMovie->getTitle();
    $genre          = $activeMovie->getGenre();
    $description    = $activeMovie->getDescription();
    $releaseDate    = $activeMovie->getReleaseDate();
    

    if(isset($_POST['txtFormType']) && $_POST['txtFormType'] === "SAVEMOVIE")
    {
        ...
        if(count($errors) == 0)
        {
            $activeMovie->setTitle($title);
            $activeMovie->setGenre($genre);
            $activeMovie->setDescription($description);
            $activeMovie->setReleaseDate($releaseDate);

            $movieDB->saveMovie($activeMovie);


            $_SESSION['Movie Saved'] = 'true';
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Editing <?php echo htmlspecialchars($activeMovie->getTitle()) ?></h1>
                </div>
                <div class="col-sm-6">
